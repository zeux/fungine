...
let buildMesh source =
    ...
    let meshes = instances |> Array.collect (fun i -> (Build.Dae.FatMeshBuilder.build doc i) |> Array.map (fun mesh -> mesh, skeleton.data.AbsoluteTransform skeleton.id_map.[i.ParentNode.Attribute "id"]))
