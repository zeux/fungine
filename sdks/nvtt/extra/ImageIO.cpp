...
Image * nv::ImageIO::loadBMP(Stream & s)
{
    nvCheck(!s.isError());
    nvCheck(s.isLoading());

    uint8*          bits;         /* Bitmap pixel bits */
    uint32          bitsize;      /* Size of bitmap */
    uint32          infosize;     /* Size of header information */
    ...
    infosize = header.bfOffBits - sizeof(BmpHeader);
    BmpInfo *bmpInfo = NULL;
    if ((bmpInfo = (BmpInfo *)::malloc(infosize)) == NULL)
    {
        /* Couldn't allocate memory for bitmap info - return NULL... */
        return (NULL);
    }

    if (s.serialize(bmpInfo, infosize) < infosize)
    {
    ...
    * the bitmap and read *it* in...                                    */
    ...
    // Allocate image.
    AutoPtr<Image> img(new Image());
    img->allocate(bmpInfo->bmiHeader.biWidth, bmpInfo->bmiHeader.biHeight);

    if ((bits = (uint8*)::malloc(bitsize)) == NULL)
    {
    ...
    // Read pixel data
    s.seek(header.bfOffBits);
    s.serialize(bits, bitsize);

    int pixelStride;
    /* 
    Leaving 1 bit bmp files as unsupported for now
    if (bmpInfo->bmiHeader.biBitCount == 1)
    {
        //Palletized image, 1bpp
        //pixelStride = 1/8;
        int padPerLine = 4 - ((bmpInfo->bmiHeader.biWidth + 7) / 8) % 4;
        //nvDebug("PAD:%d\n", padPerLine);

        for (int y = 0; y < bmpInfo->bmiHeader.biHeight; y++)
        {
            for (int x = 0; x < bmpInfo->bmiHeader.biWidth; x += 8)
            {
                long pixelPos = y * bmpInfo->bmiHeader.biWidth + x;
                long bitPos = pixelPos / 8 + padPerLine * y;
                uint8 colorByte = bits[bitPos];

                for (int bit = 0; bit < 8 && x + bit < bmpInfo->bmiHeader.biWidth; bit++)
                {
                    long pixelPosTarget = (bmpInfo->bmiHeader.biHeight - 1 - y) * bmpInfo->bmiHeader.biWidth + x + bit;
                    
                    // Shift every pixel index, and mask to 1 bit integer
                    uint8 colorIndex = (colorByte >> (8 - bit)) & 1;
                    //nvDebug("pixelPos:%d pixelPosTarget:%d bitPos:%d colorByte:%d colorIndex:%d\n", pixelPos, pixelPosTarget, bitPos, colorByte, colorIndex);
                    BmpColor& bmpColor = bmpInfo->bmiColors[colorIndex];
                    img->pixels()[pixelPosTarget].setRGBA(bmpColor.r, bmpColor.g, bmpColor.b, 255);
                }
            }
        }
    }
    ...
    */
    if (bmpInfo->bmiHeader.biBitCount == 4)
    {
        //Palletized image, 4bpp
        //pixelStride = 1/2;
        int padPerLine = bmpInfo->bmiHeader.biWidth % 4 == 0 ? 0 :
                         4 - bmpInfo->bmiHeader.biWidth % 4;
        for (int y = 0; y < bmpInfo->bmiHeader.biHeight; y++)
        {
            for (int x = 0; x < bmpInfo->bmiHeader.biWidth; x++)
            {
                long pixelPos = y * bmpInfo->bmiHeader.biWidth + x;
                long pixelPosTarget = (bmpInfo->bmiHeader.biHeight - 1 - y) * bmpInfo->bmiHeader.biWidth + x;
                long bitPos = pixelPos / 2 + padPerLine * y;
  
                // Shift every second pixel index by 4 bits, and mask to 4 bit integer
                uint8 colorIndex = (bits[bitPos] >> (((pixelPos + 1) % 2) * 4)) & 15;
                BmpColor& bmpColor = bmpInfo->bmiColors[colorIndex];
                img->pixels()[pixelPosTarget].setRGBA(bmpColor.r, bmpColor.g, bmpColor.b, 255);
            }
        }
    ...
    {
        // Palletized image, 8bpp
        int pixelStride = 1;
        int padPerLine = (3 * bmpInfo->bmiHeader.biWidth) % 4 == 0 ? 0 :
                         4 - (3 * bmpInfo->bmiHeader.biWidth) % 4;
        for (int y = 0; y < bmpInfo->bmiHeader.biHeight; y++)
        {
            for (int x = 0; x < bmpInfo->bmiHeader.biWidth; x++)
            {
                long pixelPos = y * bmpInfo->bmiHeader.biWidth + x;
                long pixelPosTarget = (bmpInfo->bmiHeader.biHeight - 1 - y) * bmpInfo->bmiHeader.biWidth + x;
                long bitPos = pixelPos * pixelStride + padPerLine * y;

                BmpColor& bmpColor = bmpInfo->bmiColors[bits[bitPos]];
                img->pixels()[pixelPosTarget].setRGBA(bmpColor.r, bmpColor.g, bmpColor.b, 255);
            }
        }
    } 
    ...
    {        
        // 16 bpp - 5r5g5b1a
        int pixelStride = 2;
        int padPerLine = (pixelStride * bmpInfo->bmiHeader.biWidth) % 4 == 0 ? 0 :
                         4 - (pixelStride * bmpInfo->bmiHeader.biWidth) % 4;
        for (int y = 0; y < bmpInfo->bmiHeader.biHeight; y++)
        {
            for (int x = 0; x < bmpInfo->bmiHeader.biWidth; x++)
            {
                ...
                uint16 pixelValue = bits[bitPos] | (bits[bitPos + 1] * 256);
                img->pixels()[pixelPosTarget].setRGBA(
                    ((pixelValue >> 0) & 31) * 255 / 31, 
                    ((pixelValue >> 5) & 31) * 255 / 31, 
                    ((pixelValue >> 10) & 31) * 255 / 31,
                    (pixelValue >> 15) * 255);                  /* 1 bit alpha */
            }
        }
    }
    else if (bmpInfo->bmiHeader.biBitCount == 24)
    {
        ...
        for (int y = 0; y < bmpInfo->bmiHeader.biHeight; y++)
        {
            for (int x = 0; x < bmpInfo->bmiHeader.biWidth; x++)
            {
                ...
                long pixelPosTarget = (bmpInfo->bmiHeader.biHeight - 1 - y) * bmpInfo->bmiHeader.biWidth + x;
                long bitPos = pixelPos * pixelStride + padPerLine * y; 
                img->pixels()[pixelPosTarget].setRGBA(bits[bitPos + 2], bits[bitPos + 1], bits[bitPos + 0], 255);
            }
        }
    }
    else if (bmpInfo->bmiHeader.biBitCount == 32)
    {
        // 32bpp
        pixelStride = 4;
        for (int y = 0; y < bmpInfo->bmiHeader.biHeight; y++)
        {
            for (int x = 0; x < bmpInfo->bmiHeader.biWidth; x++)
            {
                long pixelPos = y * bmpInfo->bmiHeader.biWidth + x;
                long pixelPosTarget = (bmpInfo->bmiHeader.biHeight - 1 - y) * bmpInfo->bmiHeader.biWidth + x;
                long bitPos = pixelPos * pixelStride;
                img->pixels()[pixelPosTarget].setRGBA(bits[bitPos + 2], bits[bitPos + 1], bits[bitPos + 0], bits[bitPos + 3]);
            }
        }
    }
    else
    {
        nvDebug( "*** ImageIO::loadBMP: Error, unsupported image type.\n" );
    }
